...
export async function getTasksByUnit(unitId: number) {
  return request<API.TaskDTO[]>('/api/tasks/unit/${unitId}');
}

// Tutor 所有分配（unitId 可选）
export async function getTutorAllocations(unitId?: number, tutorId?: number) {
  return request<API.AllocationResponse[]>("/api/allocations/tutor", {
    method: "GET",
    params: unitId ? { unitId, tutorId } : {},   // 支持 tutorId 参数
  });
}
...
export async function deleteAllocationsByTask(tutorId: number, taskId: number) {
  return request<any>(`/api/allocations`, {
    method: "DELETE",
...
export async function getAllocations(unitId: number) {
  return request<API.AllocationResponse[]>(
...
export async function getTaskTypes(unitId: number) {
  return request<any>(`/api/task-types/unit/${unitId}`);
}
...
export async function createTaskType(data: {
  ...
  return request<any>("/api/task-types", {
    method: "POST",
    data,
  });
}
...
export async function updateTaskType(id: number, data: { name: string }) {
  return request<any>(`/api/task-types/${id}`, {
    method: "PUT",
    data,
  });
}
...
export async function getTasks(unitId: number) {
  return request<any>(`/api/tasks/unit/${unitId}`);
}
